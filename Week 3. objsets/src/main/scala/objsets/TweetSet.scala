...
class Empty extends TweetSet {
    ...
    override def max(that: Tweet): Tweet = that
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    override def max(t: Tweet): Tweet = {
      val m =
        if (elem.retweets > t.retweets) elem
        else t
    ...
    override def descendingByRetweet: TweetList = {
      val fluential = mostRetweeted
      new Cons(fluential, this.remove(fluential).descendingByRetweet)
    }
  /**
   * The following methods are already implemented
   */
...
object GoogleVsApple {
  ...
  val apple = List("ios", "iOS", "iphone", "iPhone", "ipad", "iPad")
    ...
    lazy val googleTweets: TweetSet = {
      TweetReader.allTweets.filter(x => google.exists(y => x.text.contains(y)))
    }

  lazy val appleTweets: TweetSet = {
    TweetReader.allTweets.filter(x => apple.exists(y => x.text.contains(y)))
  }
